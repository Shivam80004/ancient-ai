...
const ResourceCard: React.FC<ResourceCardProps> = ({ title, description, category, image, link = "#", type, index }) => {
    ...
    return (
        ...
        >
            <div ref={cardRef} className="relative rounded-3xl overflow-hidden bg-white/5 backdrop-blur-xl border border-white/10 h-full flex flex-col hover:border-white/20 transition-colors duration-500 shadow-xl">
                {/* Image Section */}
                ...
                <div ref={contentRef} className="p-6 flex flex-col grow relative z-10">
                    <h3 className="text-xl md:text-2xl font-bold text-white mb-3 group-hover:text-amber-200 transition-colors duration-300 font-poppins leading-tight">
                        {title}
                    </h3>
                    ...
                    <div className="mt-auto pt-4 border-t border-white/10 flex items-center justify-between group-hover:border-white/30 transition-colors duration-300">
                        <span className="text-white text-xs font-semibold uppercase tracking-wider group-hover:text-amber-200 transition-colors duration-300 flex items-center gap-2">
                            {getActionLabel()}
