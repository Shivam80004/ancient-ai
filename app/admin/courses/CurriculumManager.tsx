...
import { Plus, Trash2, Eye, EyeOff, ChevronDown, Video, FileText, Loader2 } from "lucide-react";
import { GlassCard } from "@/components/dashboard/GlassCard";
import { VideoUpload } from "./VideoUpload";
import { cn } from "@/lib/utils";
import { createSemester, createCourse, createLesson, togglePublish, deleteRow } from "./actions";

export type LessonNode = { id: string; title: string; content_type: string | null };
        ...
        <div className="space-y-6">
            ...
            <GlassCard className="flex flex-wrap items-center gap-3 p-4">
                ...
                <button
                    ...
                    onClick={() =>
                        start(async () => {
                            await createSemester(title.trim());
                            setTitle("");
                            refresh();
                        })
                    }
            ...
            {open && (
                <div className="mt-3 space-y-2 border-l border-white/[0.06] pl-4">
                    {course.lessons.map((l) => (
                        <div key={l.id} className="flex items-center justify-between rounded-xl border border-white/[0.06] bg-white/[0.02] px-3 py-2">
                ...
                <button
                    ...
                    onClick={() =>
                        start(async () => {
                            ...
                            setTitle(""); setContentUrl(""); setBody(""); setDuration(0);
                            setOpen(false);
                            onDone();
                        })
                    }
